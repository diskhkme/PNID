...
def write_coco_annotation(outfile, annotation_data, symbol_dict, segment_params):
    """
    :param outfile: 출력 파일명
    :param annotation_data: 출력 annotation 데이터
    :param symbol_dict: 심볼 dictionaty (category id 매칭에 사용)
    :param segment_params: segmentation parameter ([width,height, width_stride, height_stride]
    :return:
        image_dict ["이미지 이름" : id], 후처리시 각 image_id에 해당하는 파일명을 쉽게 얻어오기 위해 같이 출력함
    """
    data = {}
    data["type"] = "instances"
